...
namespace AutoClicker.WorkWithDll.Listener
{
    public class LowLevelListener
    {
        ...
        private IntPtr KeyboardHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                bool keyDown = (wParam == (IntPtr) WmKeyDown || wParam == (IntPtr) WmSystemKeyDown);
                int vkCode = Marshal.ReadInt32(lParam);
                OnKeyPressed?.Invoke(this, new KeyPressedArgs(KeyInterop.KeyFromVirtualKey(vkCode), keyDown));
            }
        ...
        private IntPtr MouseHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                Wm inputType = (Wm) wParam;

                if (inputType == Wm.MouseMove)
                {
                ...
                else
                {
                    OnMouseMessage?.Invoke(this, 
                            new MouseArgs(MouseConverter.GetMouseMessage(inputType, lParam)));
                }
            }
